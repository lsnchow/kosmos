...
class OutboxTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_callback_finalization_projects_production_measurements_and_segments(self) -> None:
        outbox = self.make_outbox(RecordedTransport([response(202, {"request_id": "projected-result"})]))
        self.enqueue(outbox)
        await outbox.dispatch("outbox-test")
        body = callback_body("projected-result", {"run_id": self.run_id, "episode_id": self.episode_id})
        outbox.ingest_callback(body, callback_headers(body, "projected-result"))
        outbox.claim_callback_finalization("outbox-test", "projection-writer")
        result = {
            **self.completed_result(),
            ...
            "segments": [
                {
                    "segment_index": 0,
                    "frame_urls": ["/api/artifacts/frames/000.png"],
                    "certified_frame_count": 1,
        ...
        self.assertTrue(
            outbox.finalise_callback(
                ...
                "projection-writer",
                "completed",
                self.artifact("object://final/projected-result.json"),
                result=result,
            )
        )
        episode = self.ledger.list_episodes(self.run_id)[0]
        self.assertEqual((episode["n_segments"], episode["compute_gpu_seconds"]), (1, 2.5))
        self.assertEqual(episode["frame_urls"], ["/api/artifacts/frames/000.png"])
        segment_events = [
            event for event in self.ledger.list_events(self.run_id) if event["type"] == "segment_completed"
        ]
        self.assertEqual(len(segment_events), 1)
        self.assertEqual(segment_events[0]["payload"]["segment_index"], 0)
