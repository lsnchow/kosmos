...
MODEL_PATH = ROOT / "deploy" / "baseten" / "mvp_policy" / "model" / "policy_runtime.py"
...
def test_deployment_isolates_framework_requirements_and_pins_cuda_family():
    folder = MODEL_PATH.parents[1]
    config = (folder / "config.yaml").read_text()
    assert 'cpu: "4"' in config
    assert "accelerator: H100" in config
    assert "predict_concurrency: 1" in config
    assert "/opt/plumb-ml-venv" in config
    assert "tensorflow" not in (folder / "requirements.txt").read_text().lower()
    dependencies = (folder / "model" / "ml_requirements.txt").read_text()
    for pin in ("protobuf==4.25.8", "einops==0.6.1", "nvidia-cusolver-cu12==11.5.2.141",
                "nvidia-nvjitlink-cu12==12.2.140", "nvidia-cublas-cu12==12.2.5.6"):
        assert pin in dependencies


def test_native_mapping_rejects_missing_and_extra_weight_keys():
    expected = {"actor": {"weight": np.ones((2, 3), dtype=np.float32)}}
    MODULE._strict_mapping(expected, expected)
    for invalid in ({}, {"actor": {}}, {"actor": {"weight": expected["actor"]["weight"], "extra": 1}}):
        with pytest.raises(RuntimeError, match="keys differ"):
            MODULE._strict_mapping(expected, invalid)
