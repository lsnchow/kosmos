...
```json
{
  ...
  "asset_lock_hash": "sha256:<canonical-assets-lock-json>",
  "qualification_assets": {
    "world_model": ["<model-asset-id>", "<runtime/code-asset-id>"],
    "policies": {"<each canonical policy>": ["<policy-asset-id>", "<policy-code-asset-id>"]},
    "judge": ["<judge-model-asset-id>", "<judge-code-asset-id>"],
    "scenario": ["<scenario-source-asset-id>"]
  },
  "tolerances": {"gate_d": {"min_binary_kappa": 0.0, "min_weighted_progress_kappa": 0.0, "max_leniency_offset": 0.0, "min_consensus_coverage": 0.0}},
...
D. It also requires the supplied `--asset-lock` JSON to hash exactly to the
frozen `asset_lock_hash` and to contain every explicitly bound world, policy,
judge, and scenario asset as an execution-verified row. Omitting the lock,
supplying a different lock, or relying on a status-only `pass` record remains
`blocked`. The scenario content hash is recomputed, not trusted from its
declared field. Gate E is deliberately not an execution prerequisite because
it depends on the completed primary matrix; Gate F is a later demonstration
claim gate.
