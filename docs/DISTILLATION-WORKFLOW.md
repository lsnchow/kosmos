# Distilled-judge workflow

Distillation has two deliberately separate layers for a **new judge revision**:

- `plumb.distillation_preparation` freezes local development-only
  training/validation inputs and a no-submit readiness record. The `plumb
  distillation` CLI is its entrypoint.
- `plumb.distillation` owns the recorded Training Jobs specification, complete
  preregistered LoRA search results, fresh-held-out calibration, paired
  frozen-video comparison, and the `distilled_judge` Gate-E evidence mapping.
  It treats job outputs as supplied records and refuses an unqualified model
  for scoring.

Neither layer uploads data, invokes Baseten, spends money, starts a GPU, or
marks a distilled judge calibrated. A preparation artifact is not a Training
Job request; conversely, recorded job metadata cannot substitute for frozen,
development-only labels and raw teacher outputs.
...
Those annotations must retain two independently declared human owners; an
unattributed label set cannot satisfy Gate D for the new revision.
