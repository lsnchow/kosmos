...
_ENTRIES: Tuple[AssetPlanEntry, ...] = (
    ...
    AssetPlanEntry(
        ...
        license="MIT (publisher-declared in immutable README/card)",
        license_status=LicenseStatus.ADVERTISED_UNVERIFIED,
        redistribution=Redistribution.LOCAL_USE_ONLY,
        access=Access.PUBLIC,
        revision="1a4c15dd9ad780a257e9494f0fac79cbe8e64793",
        files=(
            PlannedFile(path="README.md", expected_bytes=24),
            PlannedFile(path="checkpoint/checkpoint", expected_bytes=258_718_956),
            PlannedFile(path="checkpoint/commit_success.txt", expected_bytes=162),
        ),
        max_gb=1.0,
        historical_total_estimate_bytes=258_718_956,
        required_for_primary_matrix=True,
        blockers=(),
        notes=("The immutable Hub snapshot stores the direct Flax restore file at checkpoint/checkpoint; its README advertises MIT, so preserve the card/README snapshot before relying on it.",),
    ),
    # -------------------------------------------------------------- judge and data
    AssetPlanEntry(
